...
def glue_inputs(dataset_name, batch_size, eval_batch_size=None, data_dir=None,
  ...
  if key_b is None:
    def preprocess(batch):
      ...
      return input_ids, type_ids, batch['label']
  else:
    def preprocess(batch):
      """Tokenize and convert text to model inputs."""
      batch_size = batch['idx'].shape[0]
...
def finetune(output_dir, model=gin.REQUIRED, dataset_name=gin.REQUIRED,
  ...
  trainer = Trainer(
      ...
      lr_schedule=trax.learning_rate.MultifactorSchedule,
      inputs=glue_inputs,
  ...
  def run_model(stream):
    ...
    all_out = []
    all_labels = []
    for input_ids, type_ids, labels in stream:
      ...
      out = onp.array(fwd((input_ids, type_ids)))
      if remainder:
        out = out[:-pad_amount]
      all_out.append(out)
      all_labels.append(labels)
    return onp.concatenate(all_out, axis=0), onp.concatenate(all_labels, axis=0)
  ...
  return trainer, preds_and_labels
